...
function isDuplicate(t, treinosExistentes, toleranciaMin = 10, toleranciaDist = 0.1, toleranciaDias = 1) {
  ...
  for (const existente of treinosExistentes) {
    ...
    // Levenshtein: número de alterações entre nomes (ex: <=2)
    const levDist = levenshtein.distance(
      (diaNovo.nome || t.objetivo || ''),
      (diaExistente.nome || existente.objetivo || '')
    );
    const nomeLevSimilar = levDist <= 2;

    // Jaro-Winkler: similaridade percentual (>0.85)
    const jwScore = jaroWinkler(
      (diaNovo.nome || t.objetivo || ''),
      (diaExistente.nome || existente.objetivo || '')
    );
    const nomeJWSimilar = jwScore > 0.85;

    // Tempo, distância e data igual ao exemplo anterior...
    ...
    const distSimilar = (distNovo && distExist) ? Math.abs(distNovo - distExist) <= toleranciaDist * distExist : true;
    ...
    // Se nome for similar por Levenshtein OU Jaro-Winkler + outros critérios
    if ((nomeLevSimilar || nomeJWSimilar) && tempoSimilar && distSimilar && dataSimilar && fonteIgual && tipoIgual) {
      return true;
    }
  }
  return false;
